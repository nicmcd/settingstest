...
BRANCH=cmake
BUILD_SPEC="settingstest@${BRANCH} target=${SPACK_TARGET}"
...
spack spec ${BUILD_SPEC} > /tmp/settingstest_spec_${BRANCH}

# Extracts the exact spec of a dependency then returns the install directory
function location() {
    local spec=$(grep "\^${1}@" /tmp/settingstest_spec_${BRANCH} | sed 's/\s.*^//g')
